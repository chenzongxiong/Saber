...
public class CampaignGenerator {
	public final static int numberOfCampaigns = 10000;
	...
	public CampaignGenerator (int adsPerCampaign, IPredicate joinPredicate, boolean isV2) {
		...
		this.adsArray = new long [numberOfCampaigns * adsPerCampaign][2];
		this.isV2 = isV2;
	...
	public void generateBuffer () {
		...
		SystemConf.RELATIONAL_TABLE_BUFFER_SIZE = campaignsTupleSize * numberOfCampaigns * adsPerCampaign;

		byte [] data = new byte [campaignsTupleSize * numberOfCampaigns * adsPerCampaign];
		...
		while (b1.hasRemaining()) {
			...
			campaign_id = new UUID(0L, value2%numberOfCampaigns);
	...
	private void fillAdsArray(ByteBuffer campaigns) {
		...
		/* Fill the array */
		for (int i = 0 ; i < numberOfCampaigns * adsPerCampaign; i++) {
			// fill the array with all the possible ads
