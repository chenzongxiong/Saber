...
public class NexmarkOp1 implements IOperatorCode, IAggregateOperator {
...
// 	}
    ...
    public NexmarkOp1(ITupleSchema inputSchema,
                      WindowDefinition windowDefinition,
    ...
    public void processData(WindowBatch batch, IWindowAPI api) {
		...
		for (int pointer = batch.getBufferStartPointer(); pointer < batch.getBufferEndPointer(); pointer += tupleSize) {
            ...
            outputBuffer.putFloat((float)(0.89*price));
            outputBuffer.putLong(0L); // for padding
		}

        // System.out.println("[DBG]: outputBuffer.position: " + outputBuffer.position());
