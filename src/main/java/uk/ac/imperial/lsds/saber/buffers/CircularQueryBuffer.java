...
public class CircularQueryBuffer implements IQueryBuffer {
    ...
    public CircularQueryBuffer(int id, int _size, boolean isDirect, PAPIHardwareSampler [] papiSamplers) {
		if (_size <= 0)
		...
		if (this.id == 0) {
			// numberOfThreads = 2;
            numberOfThreads = SystemConf.NUMBER_OF_CICULAR_WORKERS;
			int coreToBind = 1;
			isReady = new AtomicInteger(-1);
			workers = new CircularBufferWorker [numberOfThreads];
            for (int i = 0; i < workers.length; i++) {
                workers[i] = new CircularBufferWorker (this, i + coreToBind, papiSamplers == null ? null : papiSamplers[i]);
