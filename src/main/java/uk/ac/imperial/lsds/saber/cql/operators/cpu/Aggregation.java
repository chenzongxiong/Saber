...
public class Aggregation implements IOperatorCode, IAggregateOperator {
	...
	private void processDataPerWindow (WindowBatch batch, IWindowAPI api) {
        // System.out.println("[DBG] process data per window");
		...
		long timestampValue;
		float value;
        System.out.println("[DBG] batch.getLastWindowindex: " + batch.getLastWindowIndex());
		...
		for (int currentWindow = 0; currentWindow < startP.length; ++currentWindow) {
            ...
            System.out.println("[DBG] currentWindow: " + currentWindow + ", start: " + start + ", end: " + end + ", inputTupleSize: " + inputTupleSize + ", batch.getStreamStartPointer: " + batch.getStreamStartPointer() + ", pendingWindows.numberOfWindows(): " + pendingWindows.numberOfWindows() + ", batch.taskId: " + batch.getTaskId());
			...
			if (start < 0 && end < 0) {
                ...
                System.out.println("[DBG] completeWindows.getBuffer");
