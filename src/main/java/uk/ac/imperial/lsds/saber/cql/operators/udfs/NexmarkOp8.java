...
public class NexmarkOp8 implements IOperatorCode, IAggregateOperator {
    ...
    public NexmarkOp8(ITupleSchema inputSchema,
    ...
    public void processData(WindowBatch batch, IWindowAPI api) {
		...
		for (int pointer = batch.getBufferStartPointer(); pointer < batch.getBufferEndPointer(); pointer += tupleSize) {
            ...
            final long auction = inputBuffer.getLong(pointer+8);
            final long bidder = inputBuffer.getLong(pointer+16);
            ...
            if (auction == 1007L || auction == 2001L ||
                auction == 2019L || auction == 2087L) {
                outputBuffer.putLong(timestamp);
                outputBuffer.putLong(auction);
                outputBuffer.putLong(price);
        ...
        batch.setSchema(this.outputSchema);
...
//		}
	...
	public void processData(WindowBatch first, WindowBatch second, IWindowAPI api) {
        ...
        // System.out.println("[DBG] stream join");

		IQueryBuffer inputBuffer = first.getBuffer();
		IQueryBuffer outputBuffer = UnboundedQueryBufferFactory.newInstance();
        outputBuffer.position(0);
		ITupleSchema schema = first.getSchema();
		int tupleSize = schema.getTupleSize();
        // System.out.println("tupleSize: " + tupleSize);

		for (int pointer = first.getBufferStartPointer(); pointer < first.getBufferEndPointer(); pointer += tupleSize) {
            final long timestamp = inputBuffer.getLong(pointer);
            final long auction = inputBuffer.getLong(pointer+8);
            final long bidder = inputBuffer.getLong(pointer+16);
            final long price = inputBuffer.getLong(pointer+24);
		...
		first.setBuffer(outputBuffer);
        first.setSchema(this.outputSchema);
        // System.out.println("outputBuffer.limit: " + outputBuffer.limit());
        first.setBufferPointers(0, outputBuffer.limit());

		api.outputWindowBatchResult(first);
	}
